...
export const Diagnose = () => {
  ...
  const getDiagnose = (patient: any, symptom: any) => {
    const fetchData = async () => {
      ...
      try {
        const response = await diseaseService.getDiagnose(patient, symptom);
  ...
  if (pageType == 1) {
      ...
      <>
        <div className="h-full flex justify-center items-center flex-col">
              ...
              return (
                <button
                  ...
                  className={`btn btn-wide ${
                    checkIfSymptomExist(id) ? "btn-primary" : ""
                  }`}
          ...
          <div className="flex flex-row max-sm:flex-col gap-4 mt-10">
            ...
            >
              {endIndex >= symptomList.length ? "Diagnose" : "Lanjutkan"}
            </button>
          </div>
        </div>
      </>
    ...
    if (typeof diagnose === "undefined") {
      ...
      const sortedKeys = Array.from(Object.keys(diagnose.result)).sort(
        (a, b) => diagnose.result[b].percentage - diagnose.result[a].percentage
      );
            ...
            >
              <div className="card-body">
                <div className="flex flex-col gap-2">
                  ...
                  <p>
                    <b>Percentage:</b>{" "}
                    <b className="text-lime-500">{percentage.toFixed(2)} %</b>
                  </p>
                  <p>
                    <b>
                      Definisi: <br></br>
                  ...
                  <div className="flex flex-row flex-wrap gap-2">
                    {symptoms.map((symptom: any, index: number) => {
                      const symp = symptomList.find(
                        (item: any) => item.id === symptom
                      );
                      return (
                        <div
                          key={index + symp.name}
                          className="badge badge-md h-fit"
                        >
                          {symptom ? symp.name : "Symptom not found"}
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-row flex-wrap gap-2">
                      {diagnose.initial_symptoms.map((item: any) => {
                        const symptom = symptomList.find(
                          (symptom: any) => symptom.id === item
                        );
                        return (
                          <>
                            {" "}
                            <div className="badge badge-md gap-2">
                              {symptom ? symptom.name : "Symptom not found"}
                            </div>
                          </>
                        );
                      })}
