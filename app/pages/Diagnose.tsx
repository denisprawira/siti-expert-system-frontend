...
export  const Diagnose = ()=>{
    ...
    const [diagnose, setDianose] = useState<any>(undefined);
    ...
    const removeSymptom = (id:number) =>{
        setSelectedSymptoms(symptomData => symptomData.filter(symptomId => symptomId !== id));
    }
    ...
    const getDiagnose = (data:any)=>{
        const fetchData = async ()=>{
            const diseaseService = new DiseaseService();
            try{
                const response = await diseaseService.getDiagnose(data);
    ...
    if(pageType==1){
                ...
                <div className="flex flex-col justify-center max-sm:mx-4">
                    <form className="flex flex-col items-center gap-2" onSubmit={handleSubmit}>
                        <div className="form-control w-full max-w-lg">
                            ...
                            <input name="name" type="text" placeholder="Nama" className="input input-bordered w-full max-w-lg" />
                        </div>

                        <div className="form-control w-full max-w-lg">
                            <label className="label">
                                <span className="label-text">Pilih Jenis Kelamin Anda</span>
                            </label>
                            <select name="sex" className="select select-bordered">
                        ...
                        <div className="form-control w-full max-w-lg">
                            <label className="label">
                                <span className="label-text">Masukkan Umur Anda</span>
                            </label>
                            <input name="age" type="number" placeholder="Umur" className="input input-bordered w-full max-w-lg" />
                        </div>

                        <div className="form-control w-full max-w-lg">
                            <label className="label">
                                <span className="label-text">Masukkan Alamat Tempat Tinggal Anda</span>
                            </label>
                            <input name="address" type="text" placeholder="Alamat" className="input input-bordered w-full max-w-lg" />
                        </div>
                        ...
                        <div className="form-control w-full max-w-lg">
                            ...
                            <input name="email" type="text" placeholder="Email" className="input input-bordered w-full max-w-lg" />
                        </div>
        ...
        return (
            <>
                <div className="h-full flex justify-center items-center flex-col">
                    <h1 className="text-2xl mb-12">Silahkan Pilih Gejala yang Dirasakan: </h1>
                    <div className="grid lg:grid-rows-4 lg:grid-cols-3 gap-4 max-sm:grid-rows-9 max-sm:grid-cols-1 sm:grid-rows-4 sm:grid-cols-2">
                        {currentItems.map((symptom:any,index:any)=>{
                            let id = symptom['id'];
                            return(
                                <button key={id} className={`btn btn-wide ${checkIfSymptomExist(id)? 'btn-primary':''}`}  onClick={()=>toggleSymptom(id)}>{symptom['name']}</button>
                            )}
                        )}
                    </div>
                    <div className="divider"></div> 
                <div className="flex flex-row max-sm:flex-col gap-4 mt-10" >
                    <button  id="btnDiagnose" className="btn btn-wide btn-success" onClick={
                        ()=>{
                    ...
                    >{endIndex >= symptomList.length? "Diagnose":"Lanjutkan"}</button>
        ...
        if (typeof diagnose === 'undefined') {
            ...
            const sortedKeys = Array.from(Object.keys(diagnose.result)).sort((a, b) => diagnose.result[b].percentage - diagnose.result[a].percentage);
            const resultElements = sortedKeys.map((key) => {
                ...
                return (
                <>
                    <div key={key} className="card w-96 bg-base-100 shadow-xl mb-4 w-full ">
                        <div className="card-body">
                            <div className="flex flex-col gap-2">
                            <h1 className="text-rose-800 text-lg"><b>{disease.name}</b></h1>
                            <p><b>Percentage:</b> <b className="text-lime-500">{percentage.toFixed(2)} %</b></p>
                            <p><b>Definisi: <br></br></b>{disease.definition}</p>
                            <p><b>Initial Symptom:</b> </p>
                                <div className="flex flex-row flex-wrap gap-2">
                                    {symptoms.map((symptom:any, index:number) => {
                                        const symp = symptomList.find((item: any) => item.id === symptom);
                                        return ( <div className="badge badge-md h-fit">{symptom ? symp.name : 'Symptom not found'}</div>)
                                    })}
                                </div>
              ...
              const [firstElement, secondElement, ...remainingElements] = resultElements;
                    ...
                    <div className="flex flex-col max-lg:m-4 max-lg:mb-4">
                        {patient !== null && patient !== undefined ? (
                            <div className="card w-96 bg-base-100 shadow-xl mb-4 w-full ">
                                <div className="card-body">
                                    <div className="flex flex-col gap-2">
                                    <h1 className="text-rose-800 text-lg"><b>Data Pasien</b></h1>
                                    <p><b>Nama Pasien   : </b> {patient.name}</p>
                                    <p><b>Umur          : </b>{patient.age}</p>
                                    <p><b>Jenis Kelamin : </b> {patient.sex}</p>
                                    <p><b>alamat        : </b>{patient.address}</p>
                                    <p><b>Email         : </b> {patient.email}</p>
                                    <h1><b>Gejala Awal: </b></h1>
                               
                                    <div className="flex flex-row flex-wrap gap-2">
                                        {diagnose.initial_symptoms.map((item:any)=>{
                                            const symptom = symptomList.find((symptom: any) => symptom.id === item);
                                            return <> <div className="badge badge-md gap-2">{symptom ? symptom.name : 'Symptom not found'}</div></>;
                                        })}
                                    </div>
                                    </div>
                                </div>
